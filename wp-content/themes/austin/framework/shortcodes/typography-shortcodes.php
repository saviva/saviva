...
function mo_blockquote_shortcode($atts, $content = null, $shortcode_name = "") {
    ...
    if (!empty($subtitle)) {
        $subtitle = '<div class="subtitle">' . htmlspecialchars_decode($subtitle) . '</div>';
    }

    $output = '<blockquote ' . $id . ' class="align' . $align . $class . '"' . $style . '>' . $subtitle . do_shortcode($content) . $author_info . '</blockquote>';
...
button_url - A button is displayed on top right of the heading. The button serves as a link to related information. This parameters captures the URL linked to by the button.
button_text - The title of the button shown as part of the heading.
...
function mo_heading2_shortcode($atts) {
    ...
    if (!empty ($button_url))
        $output .= '<a class="button" title="' . $button_text . '" href="' . $button_url . '">' . $button_text . '</a>';
    if (!empty ($subtitle))
        $output .= '<div class="subtext">' . $subtitle . '</div>';
    if (!empty ($title))
        $output .= '<h3 class="title">' . htmlspecialchars_decode($title) . '</h3>';
    if (!empty ($pitch_text))
        $output .= '<div class="pitch">' . htmlspecialchars_decode($pitch_text) . '</div>';
